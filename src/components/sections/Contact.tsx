...
const Contact: FC<ContactProps> = ({ name }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    // Basic validation
    if (formData.name && formData.email && formData.message && formData.captcha === '27') {
      setIsSubmitted(true);
      // Reset form
      setFormData({
        name: '',
    ...
    <section className="py-16 bg-gray-50" id={name}>
      <div className="container mx-auto px-4">
        <div className="max-w-4xl mx-auto grid md:grid-cols-2 gap-8">
          {/* Contact Information */}
          <div>
            <h2 className="text-3xl font-bold text-[#2C5282] mb-6">Contact Us</h2>
